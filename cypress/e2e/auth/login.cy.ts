describe('Login Page', () => {
  ...
  it('should show validation errors for empty fields', () => {
    ...
    cy.get('p.text-destructive').first().should('contain.text', 'Email is required');
    cy.get('p.text-destructive').last().should('contain.text', 'Password is required');
  });
  ...
  it('should show validation error for short password', () => {
    ...
    cy.get('button[type="submit"]').click();
    cy.get('p.text-destructive').should('contain.text', 'Password must be at least 6 characters');
  });
  ...
  it('should handle failed login', () => {
    ...
    cy.get('[role="alert"]').should('contain.text', 'Invalid login credentials');
  });
});
